      ...
      {TABS.map(({ key, label, icon: Icon }) => {
        ...
        return (
          <button
            ...
            className={`tap flex flex-col items-center gap-0.5 rounded-xl px-3.5 py-1.5 text-[10px] transition-all duration-200 ${
              isActive ? "bg-[var(--vco-green-soft)] text-[var(--vco-text)] shadow-[0_2px_8px_-3px_var(--vco-green)]" : "text-[var(--vco-text-faint)]"
            }`}
          >
            <Icon
              size={19}
              className={`transition-transform duration-300 ease-out ${isActive ? "-translate-y-0.5 scale-110 stroke-[var(--vco-green-strong)]" : "stroke-[var(--vco-text-faint)]"}`}
            />
            {label}
          </button>
        );
